...
-(void)addArray:(NSArray *)value {
    @synchronized (value) {
        ...
        dispatch_async(dispatch_get_main_queue(),  ^{
            self.progressCount =  _progressCount + 1;
...
-(void)sendRequestForUserMediaAndMyID:(NSString *)myID {
    NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
    NSURLSession *session = [NSURLSession sessionWithConfiguration:configuration delegate: self delegateQueue:nil];
    NSMutableURLRequest *requestData = [NSMutableURLRequest requestWithURL:
                                       [NSURL URLWithString:
...
-(void)saveMediaDictionary:(NSDictionary *)mediaDict {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.userStandartPhotoUrlArray = [self.userMediaDictionary valueForKeyPath:@"data.images.standard_resolution.url"];
